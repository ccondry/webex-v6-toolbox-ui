// loading and working states
export const SET_WORKING = 'SET_WORKING'
export const SET_LOADING = 'SET_LOADING'

// Authentication REST API version
export const SET_AUTH_API_VERSION = 'SET_AUTH_API_VERSION'

// user JWT
export const SET_JWT = 'SET_JWT'

// links list
export const SET_LINKS = 'SET_LINKS'

// user details
export const SET_USER = 'SET_USER'
// demo verticals list
export const SET_VERTICALS = 'SET_VERTICALS'
// full vertical details for current vertical ID
export const SET_VERTICAL_DETAILS = 'SET_VERTICAL_DETAILS'
// dcloud session ID and datacenter
export const SET_INSTANCE = 'SET_DCLOUD_SESSION'

// user agent desktop layout
export const SET_DESKTOP_LAYOUT = 'SET_DESKTOP_LAYOUT'
// global agent desktop layout
export const SET_GLOBAL_DESKTOP_LAYOUT = 'SET_GLOBAL_DESKTOP_LAYOUT'
// dcloud demo base configuration
export const SET_DEMO_INFO = 'SET_DEMO_INFO'
// JDS identity
export const SET_JDS_IDENTITY = 'SET_JDS_IDENTITY'

// outbound
export const SET_OUTBOUND_SEND_ONE_RESPONSE = 'SET_OUTBOUND_SEND_ONE_RESPONSE'
export const SET_OUTBOUND_UPLOAD_RESPONSE = 'SET_OUTBOUND_UPLOAD_RESPONSE'
export const SET_OUTBOUND_PROVISION_STATUS = 'SET_OUTBOUND_PROVISION_STATUS'